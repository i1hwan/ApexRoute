...
Production traffic on 3.8.6 / 3.8.5 showed a `feat/limits-dashboard-polish` smart-affinity violation: an `Account A → Account B` switch fired only 4 seconds after the last successful Account A request, on the same opencode-supplied `x-session-affinity` header, with both accounts at ~100% quota. The cache rebuilt from scratch (147k token cache_write, cache_read=0). Oracle hard-debug (`ses_1fa7165c0ffeFFU8rjU82y0ItO`) identified three independent design defects, all fixed here.
...
- **Copilot PR #28 R3 review** (2 defects). **R3-1**: the deprecated 2-arg `touchSession(sessionId, connectionId)` compat shim previously updated `lastActive` before forwarding to `bindSessionConnection`, which made the forwarded call always see "within affinity window" — a session idle >5 min would mis-fire the within-window alarm on its first compat-shim rebind. The shim now binds first (so the binding decision sees the pre-touch `lastActive`), then preserves the original 1-touch == 1-request semantics by only incrementing `requestCount` if an entry pre-existed (since `bindSessionConnection`'s `first_bind` branch already sets it to 1). **R3-2**: `selectByEarliestResetFirst` previously bound the fall-through selection itself with a hard-coded `source: "fall_through"`. That meant the actual within-window rebind happened in the strategy with the wrong source, so the `emergency_fallback` alarm suppression in `chat.ts:548` could never apply (the strategy's bind beat the post-credential bind). The strategy no longer binds — `chat.ts:548` is now the single binding site, and it picks `source: "emergency_fallback"` vs `"explicit_post_credential"` from `runtimeOptions.emergencyFallbackTried`.
...
- **Routing Strategy v7 — burn-rate pressure max replaces v6 mean:** v6 combined session and weekly track scores via arithmetic mean, which dilutes the more-urgent track's signal with the calmer track. Production observation: `Account A(33%/0h34m, 91%/6d17h)` was picked over `Account B(29%/1h54m, 63%/1d1h)` even though Account B's weekly was about to waste 63% in ~1 day. v7 introduces `pressure(Q, t, W) = Q × clamp(W/t, 1, URGENCY_CAP)` per track and combines via `Math.max(...)`. Two windows are independent ledgers (verified via anthropics/claude-code issues #54750, #52135, #40513), so routing to the more perishable signal matches the user's "burn what's about to expire" intent.
